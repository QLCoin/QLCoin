...
uint256 hashGenesisBlock("0x3a105d312952f7bf8a5dfa02684c9a8b2763ccdd8f9ab0b73ad154ca25a28445");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // qlcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "qlcoin Signed Message:\n";
...
{
    // qlcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan =  60 * 60; // qlcoin: 
static const int64 nTargetSpacing = 60 ; // qlcoin: 
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // qlcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // qlcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // qlcoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "qlcoin is a CBigNum nLockTime system qlcoin by Satoshi Nakamoto in 2008.";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1456539942;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2095845290;
        ...
        assert(block.hashMerkleRoot == uint256("0xf64b0c3086d35eebcd7c13e6c0bbf380a747dc707d0a33dd620416807108c2ea"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // qlcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("qlcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("qlcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static qlcoinMiner(CWallet *pwallet)
{
    printf("qlcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("qlcoin-miner");
    ...
    try { loop {
        ...
        printf("Running qlcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&qlcoinMiner, pwallet));
}
